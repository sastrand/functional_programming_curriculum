...
performance cost. In the `fact` and `zip_add` functions above, we make n recursive
...
iterative implementation of `zip_add` uses O(n) space, we could improve this if we
